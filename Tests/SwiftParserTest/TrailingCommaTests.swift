...
final class TrailingCommaTests: ParserTestCase {
    ...
    func testSandbox() {
        ...
        assertParse(
            "1️⃣guadr true else { }",
            diagnostics: [DiagnosticSpec(message: "did you mean to use `guard` keyword?", fixIts: ["replace `guadr` with `guard`"])],
            fixedSource: "guard true else { }"
        )
        
        assertParse(
            "1️⃣whle true { }",
            diagnostics: [DiagnosticSpec(message: "did you mean to use `while` keyword?", fixIts: ["replace `whle` with `while`"])],
            fixedSource: "while true { }"
        )
    }
    
    func testArgumentList() {
        assertParse("ife(1, 2, 3,)", experimentalFeatures: .trailingComma)
